...
alias lll='ls $LS_OPTIONS -alh --time-style=long-iso'
...
alias flink='readlink -f'

sstart () { systemctl start "$1" --no-pager -l && systemctl status "$1" --no-pager -l; }
sstop () { systemctl stop "$1" --no-pager -l && systemctl status "$1" --no-pager -l; }
